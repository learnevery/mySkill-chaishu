...
2.8. **知识库回查**（如 Phase 0 检测到知识库）— 按大纲本章「爽点设计」「悬念钩子」两列的选型，到 `爽点.md` / `范式.md` 读取对应条目的具体写法：爽点的铺垫→释放结构、钩子的样式与强度，落实到本章正文（详见 [kb-integration.md](kb-integration.md)）。**只取写法，不取情节**
...
**多 Agent 协作模式（平台特有，需手动开启）**。通过 TeamCreate 创建写作团队，Agent 之间可以通讯协作。
